...
def _write_log(output: Path, row: dict) -> None:
    output.mkdir(parents=True, exist_ok=True)
    with open(output / "metrics.jsonl", "a", encoding="utf-8") as file:
...
def train_model(cfg, checkpoint: str | None = None) -> dict:
    ...
    device, amp_dtype = _device(cfg), _amp_dtype(cfg, _device(cfg))
