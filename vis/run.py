"""CLI 入口：流场样本、训练曲线与模型推理可视化（留在 vis 包根）

模块: vis/run.py
依赖: config, vis.flowfield, vis.inference, vis.training_curves
...
def main() -> None:
    ...
    if args.training_curves and args.indices is not None:
        parser.error("--indices 不适用于 --training-curves")
