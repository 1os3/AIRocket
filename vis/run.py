"""CLI 入口：流场样本与训练曲线可视化（留在 vis 包根）

模块: vis/run.py
依赖: config, vis.flowfield, vis.training_curves
...
def main() -> None:
    parser = argparse.ArgumentParser(description="流场样本与训练曲线可视化")
    parser.add_argument("--env", default=None, help="环境覆盖 yaml（如 config/smoke.yaml）")
    target = parser.add_mutually_exclusive_group()
    target.add_argument("--indices", type=int, nargs="+", default=None,
